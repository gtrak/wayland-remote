...
tags: [tokio, tcp, binary-protocol, streaming, bytes]

# Dependency graph
requires:
  - phase: 03-headless-rendering
    provides: captured_frames HashMap<ObjectId, RgbaData> for streaming source
provides:
...
affects: [tcp-transmission, viewer-rendering, frame-capture]

# Tech tracking
tech-stack:
  added: [bytes 1.9]
  patterns: [20-byte big-endian header, Arc<RwLock<HashMap>> for shared state, Tokio TcpListener with calloop integration]
...
patterns-established:
  - "encode_frame() + decode_header() for binary protocol"
  - "get_frames_for_streaming() converts ObjectId to window_id mapping"
  - "calloop::futures::Executor pattern for Tokio integration"
