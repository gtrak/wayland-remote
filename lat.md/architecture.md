...
Rendering is gated by a per-window dirty flag (`Window::dirty`, [[crates/server/src/window.rs#Window]]): a window renders only while it may have new pixels. It starts dirty at map; `CompositorHandler::commit` ([[crates/server/src/state.rs#State]]) calls `WindowManager::mark_all_mapped_dirty` ([[crates/server/src/window.rs#WindowManager#mark_all_mapped_dirty]]) on any new-buffer commit, and `inject` ([[crates/server/src/input/mod.rs#inject]]) does the same on pointer move (the in-frame cursor must keep moving until it moves to the viewer). The stream loop in `run` ([[crates/server/src/lib.rs#run]]) consumes the flag via `WindowManager::take_dirty` ([[crates/server/src/window.rs#WindowManager#take_dirty]]) and skips clean windows. A just-mapped window renders exactly once, then stays clean until the next new-buffer commit or pointer move, so a static window pays no GL import / PBO readback / compress while idle; animating clients commit a new buffer each frame and keep re-rendering. See [[decisions#Decision Log#Per-window change gating]].

The pointer cursor is composited on top of the focused window. `SeatHandler::cursor_image` stores the `wl_pointer.set_cursor` surface in `State.cursor_surface` (cleared on `Hidden`/`Named` and on surface destroy); `render_window` reads the pointer's current focus and location, offsets by the cursor hotspot (`CursorImageSurfaceData`), and passes the cursor to `render_window_surface`, which draws it last so it sits above window content. The cursor surface is excluded from surface tiling (`get_role == "cursor_image"`) so it is not double-drawn.
